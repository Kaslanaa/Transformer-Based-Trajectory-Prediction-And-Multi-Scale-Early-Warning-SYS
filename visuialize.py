...
plt.grid(ls='--')
plt.xlabel('Detected X value',fontweight='bold')
plt.ylabel('Detected Y value',fontweight='bold')
plt.title('SOTA-TP-SYS For Multi-Scale Prediction AND Multi-Alerts',fontweight='bold')
...
Source_point = plt.plot(x_current, y_current, 'g', alpha=0.8, marker='o',label='Source Point (X1,Y1)') # 当前真实轨迹的最后一个点
Source_point_plus = plt.plot(x_current_plus, y_current_plus, 'r' , alpha=0.8, marker='o', label='Source Point (X2,Y2)')
# plt.legend(handles=[Source_point[0], Source_point_plus[0]], labels=['Source Point'], handler_map={tuple: HandlerTuple(ndivide=None)})

First_line = plt.plot([x_current,predicted_traj[0,0]], [y_current,predicted_traj[0,1]], linestyle='--', color='green',alpha=0.2,label='Predicted Trajectory Line')
First_line_plus = plt.plot([x_current_plus,predicted_traj_plus[0,0]],[y_current_plus,predicted_traj_plus[0,1]],linestyle='--',color='black',alpha=0.2,label='Predicted Trajectory Line')
...
plt.plot(-8.775,41.4,color = 'purple',alpha=1,marker='8',label='Confidential point*')#机密点1
plt.plot(-8.825,41.25,color='purple',alpha=1,marker='^')#机密点2
ax.text(-8.89,41.01,'Demo.',fontsize=12, fontweight='bold')
ax.text(-8.88,41.32,'ALARM AREA',fontsize=10, fontweight='bold')

predict_point_ani = plt.scatter(x_cnt, y_cnt, c='b', alpha=0.4, marker='o',label='Predicted Trajectory point 1')  # 预测轨迹的第一个点
predict_point_ani_plus = plt.scatter(x_cnt_plus,y_cnt_plus,c='y',alpha=0.4,marker='o', label='Predicted Trajectory point 2')

x_text_ani_prediction = plt.text(-8.62, 41.05, '', fontsize=9,fontweight='bold')  # 预测轨迹的x标签
x_text_ani_prediction.set_text('Predicted X1:%.3f' % predicted_traj[0, 0])
y_text_ani_prediction = plt.text(-8.62, 41.02, '', fontsize=9,fontweight='bold')  # 预测轨迹的y标签
y_text_ani_prediction.set_text('Predicted Y1:%.3f' % predicted_traj[0, 1])
x_text_ani_prediction_plus = plt.text(-8.85,41.09,'',fontsize=9,fontweight='bold')
x_text_ani_prediction_plus.set_text('Predicted X2:%.3f'%predicted_traj_plus[0,0])
y_text_ani_prediction_plus = plt.text(-8.85,41.06,'',fontsize=9,fontweight='bold')
...
ax.add_patch(arrow_annotation)
ax.add_patch(arrow_annotation_plus)

circle1 = patches.Circle((-8.775,41.4), 0.05, linestyle='dashed',edgecolor='red',alpha=0.4, facecolor='none',label='Warning area(Base)')
...
circle4 = patches.Circle((-8.825,41.25), 0.05, linestyle='dashed',edgecolor='red',alpha=0.4, facecolor='none')
...
def update(num):
    if num > 0:#从第二个预测轨迹点开始
        ...
        plt.plot([predicted_traj_plus[num - 1, 0], predicted_traj_plus[num, 0]],
                 [predicted_traj_plus[num - 1, 1], predicted_traj_plus[num, 1]], linestyle='--', color='black',
                 alpha=0.2)
        plt.plot([predicted_traj[num-1, 0], predicted_traj[num, 0]], [predicted_traj[num-1, 1], predicted_traj[num, 1]], linestyle='--', color='green',alpha=0.2)

    return [predict_point_ani, x_text_ani_prediction, y_text_ani_prediction, arrow_annotation, predict_point_ani_plus, x_text_ani_prediction_plus, y_text_ani_prediction_plus, arrow_annotation_plus ]

ani = animation.FuncAnimation(fig=fig, func=update, frames=np.arange(0, 7), interval=1500, repeat=False)
ani.save('animation.gif', writer='pillow',dpi=800)
